...
void XsensDataParser::parse(uint8_t d[], int l) {
	...
	while(c<len && c<l) {
		...
		uint8_t dataLen = d[c++];
		switch(dataId) {
			case UTC_TIME: parseUtcTime(d, c); PRINT("A"); break;
			case PACKET_COUNTER: parsePacketCounter(d, c); PRINT("B"); break;
			case EULER_ANGLES: parseEulerAngles(d, c); PRINT("C"); break;
			case ACCELERATION: parseAcceleration(d, c); PRINT("D"); break;
			case STATUS_WORD: parseStatusWord(d, c); PRINT("E"); break;
			case GNSSPVT: parseGnssPvt(d, c); PRINT("F") break;
			case VELOCITYXYZ: parseVelocityXYZ(d, c); PRINT("G") break;
//			case ALTITUDE: parseAltitude(d, c); break;
			case ROT: parseRot(d, c); PRINT("H") break;
			case BAROPRESSURE: parseBaroPressure(d, c); PRINT("I") break;
		}
		c += dataLen;
	}
}
