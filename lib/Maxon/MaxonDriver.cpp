...
MaxonDriver::MaxonDriver(CanManager *canMaxon, CanManager *canManager, uint16_t nodeID) {
        ...
        this->heightOn = false;
        this->ready = false;
        this->maxonMotor = new MaxonMotor(this->canMaxonBuffer, this->canMaxon, nodeID);
}

void MaxonDriver::thread () {

        PRINT("\n\rMaxon thread started.\n");
...
void MaxonDriver::setup() {
        ...
        maxonMotor->disableDevice();                  //TODO: Gives SetShutdown error
...
void MaxonDriver::updateParameters() {       //TODO: Write without flash
