...
MaxonMotor::MaxonMotor(CanBuffer* buffer, CanManager* canManager, uint16_t nodeID)
...
MaxonMotorException MaxonMotor::resetCommunication(){
	char dataNmtResetCommunication[2] = {0x82, this->nodeID};
	WriteCanException ex = canManager->writeCan(0x00, dataNmtResetCommunication, 2);
	if (ex == WriteCanException::timeout){
		return MaxonMotorException::timeout;
	}
	else {return MaxonMotorException::noException;}
}
